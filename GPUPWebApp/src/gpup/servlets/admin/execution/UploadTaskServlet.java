...
@WebServlet(name = "UploadTaskServlet", urlPatterns = {"/admin/upload-task"})
public class UploadTaskServlet extends HttpServlet {
    ...
    private void processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        ExistExecutionConfigDTO existExeConfigDTO = null;
        ExecutionConfigDTO executionConfigDTO = null;
        String isGraphTask = request.getParameter(GRAPH_TASK);
        Scanner s = new Scanner(request.getInputStream(), "UTF-8").useDelimiter("\\A");
        String reqBodyAsString = s.hasNext() ? s.next() : null;

        if (isGraphTask.equals(YES)) {
            executionConfigDTO = parseBodyToExecutionConfigDTO(reqBodyAsString);
        } else if (isGraphTask.equals(NO)) {
            existExeConfigDTO = parseBodyToExistExeConfigDTO(reqBodyAsString);
        }
        if (executionConfigDTO != null || existExeConfigDTO != null) {
                ...
                try {
                    synchronized (this) {
                        if (isGraphTask.equals(YES)) {
                            ...
                            engine.addExecution(existExeConfigDTO);
                        }
                    }
    ...
    private ExistExecutionConfigDTO parseBodyToExistExeConfigDTO(String reqBodyAsString) {
        if (reqBodyAsString != null || !reqBodyAsString.isEmpty()) {
            JsonObject jsonObject = JsonParser.parseString(reqBodyAsString).getAsJsonObject();
            ExistExecutionConfigDTO exeConfig = new ExistExecutionConfigDTO();

            if (jsonObject.has("baseName")) {
                exeConfig.setBaseName(jsonObject.get("baseName").getAsString());
            }
            if (jsonObject.has("isIncremental")) {
                exeConfig.setIncremental(jsonObject.get("isIncremental").getAsBoolean());
            }
            return exeConfig;
        }
        return null;
